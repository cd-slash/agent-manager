...
export function ImplementationTab({
  ...
  const handleStartImplementation = async () => {
    ...
    try {
      ...
      toast.success('Implementation started', 'The implementation agent has begun coding');
    } catch (error) {
      toast.error('Failed to start', error instanceof Error ? error.message : 'Failed to start implementation phase');
    } finally {
      setIsStarting(false);
    }
  };
      ...
      {isPending && (
        <section>
          <div className="bg-surface border border-border rounded-lg p-6 text-center">
            <Code2 size={48} className="mx-auto mb-4 text-muted-foreground opacity-50" />
            <h4 className="text-lg font-medium text-foreground mb-2">
              Ready to Start Implementation
            </h4>
            <p className="text-sm text-muted-foreground mb-6 max-w-md mx-auto">
              The implementation agent will write code based on the planning output,
              create commits, and open a pull request when complete.
            </p>
          ...
          <section>
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-6">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <span className="text-lg font-medium text-foreground">Implementation in Progress</span>
              </div>
              <p className="text-sm text-muted-foreground text-center">
                The agent is writing code and making changes. A pull request will be created when complete.
              </p>
            ...
            <section>
              ...
              <div className="bg-surface border border-border rounded-lg p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="default" className="uppercase">Running</Badge>
                </div>
              </div>
            </section>
          )}
        ...
        <section>
          <div className="bg-success/5 border border-success/20 rounded-lg p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              The agent has completed the implementation. View the code changes in the Diff tab
              or proceed to review the Pull Request.
            </p>
          </div>
        </section>
      )}
        ...
        <section>
          <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-6">
            <div className="flex items-start gap-4">
              <Code2 size={24} className="text-destructive flex-shrink-0 mt-1" />
