...
export function PlanningTab({
  ...
  const handleStartPlanning = async () => {
    ...
    try {
      ...
      toast.success('Planning started', 'The planning agent has begun analyzing the task');
    } catch (error) {
      toast.error('Failed to start', error instanceof Error ? error.message : 'Failed to start planning phase');
    } finally {
      setIsStarting(false);
    }
  };
        ...
        <section>
          <div className="bg-surface border border-border rounded-lg p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6 max-w-md mx-auto">
              The planning agent will analyze the task requirements and generate acceptance criteria,
              implementation prompt, and test cases.
            </p>
        ...
        <section>
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-6">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <span className="text-lg font-medium text-foreground">Planning in Progress</span>
            </div>
            <p className="text-sm text-muted-foreground text-center">
              The planning agent is analyzing the task requirements. This may take a few minutes.
            </p>
          </div>
        </section>
      )}
        ...
        <>
          <section>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3 flex items-center">
              <CheckSquare size={16} className="mr-2" /> Generated Acceptance Criteria
            </h3>
            <div className="bg-surface border border-border rounded-lg overflow-hidden">
              {acceptanceCriteria.length === 0 ? (
                <div className="p-4 text-sm text-muted-foreground">
                  No acceptance criteria generated. Run the planning phase to generate criteria.
                </div>
              ) : (
          ...
          <section>
            ...
            <div className="bg-surface border border-border rounded-lg overflow-hidden">
              {tests.length === 0 ? (
                <div className="p-4 text-sm text-muted-foreground">
                  No test cases generated. Run the planning phase to generate tests.
                </div>
              ) : (
                tests.map((test) => (
                  <div
          ...
          {implementationPrompt && (
            <section>
              ...
              >
                ...
                <FileText size={16} className="mr-2" /> Implementation Prompt (Generated)
              </button>
              {showPrompt && (
