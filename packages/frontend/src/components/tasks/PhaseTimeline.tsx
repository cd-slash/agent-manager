import { Check, Circle, Clock, AlertCircle, SkipForward, ChevronRight } from 'lucide-react';
...
function getConnectorColor(fromStatus: PhaseStatus, toStatus: PhaseStatus): string {
  if (fromStatus === 'completed' || fromStatus === 'skipped') {
    if (toStatus === 'completed' || toStatus === 'in_progress' || toStatus === 'skipped') {
      return 'bg-success';
    }
  }
  return 'bg-border';
...
export function PhaseTimeline({ phases, currentPhase, onPhaseClick, compact = false }: PhaseTimelineProps) {
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom">
                <div className="space-y-1">
                  ...
                  {record?.model && (
                    <p className="text-xs text-muted-foreground">Model: {record.model}</p>
                  )}
...
export function PhaseTimelineMini({ phases, currentPhase }: Pick<PhaseTimelineProps, 'phases' | 'currentPhase'>) {
  const phaseMap = new Map(phases.map((p) => [p.phase, p]));
