...
import { Settings2, RotateCcw, Save, RefreshCw } from 'lucide-react';
...
export function PhaseConfigPanel({ taskId, phase, isOpen, onClose, readOnly = false }: PhaseConfigPanelProps) {
  ...
  const effectiveConfig = useQuery(api.phaseConfigs.getEffectiveConfig, { taskId, phase });
  ...
  const handleSave = async () => {
    ...
    try {
      if (useDefault) {
        ...
        toast.success('Configuration updated', 'Using default settings for this phase');
      } else {
        // Save override
        ...
        toast.success('Configuration saved', 'Custom settings applied for this phase');
      }
      setIsDirty(false);
    } catch (error) {
  ...
  const handleResetToBuiltin = () => {
    const builtinConfig = builtinDefaults?.configs?.[phase as keyof typeof builtinDefaults.configs];
    const builtinPrompt = builtinDefaults?.prompts?.[phase as keyof typeof builtinDefaults.prompts];
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className={!useDefault ? '' : 'opacity-50 pointer-events-none'}>
            ...
            <div className="space-y-2 mt-4">
              ...
              <p className="text-xs text-muted-foreground">
                Variables: {'{{task.title}}'}, {'{{task.description}}'}, {'{{task.id}}'}, {'{{pr.number}}'}, {'{{pr.branch}}'}
              </p>
            </div>
          ...
          {effectiveConfig?.isOverride && (
            <div className="text-xs text-muted-foreground bg-surface border border-border rounded p-2">
              This task has custom settings for this phase. Toggle "Use Default Settings" to revert to global defaults.
            </div>
          )}
