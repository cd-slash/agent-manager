...
function RemediationCycleCard({
	cycle,
	isCurrentCycle,
}: RemediationCycleCardProps) {
				...
				<CollapsibleTrigger asChild>
					...
					>
						...
						<div className="flex items-center gap-3">
							...
							<Badge
								className={`${statusColors[cycle.status]} uppercase text-xs`}
							>
...
export function RemediationTab({
	...
	const handleStartRemediation = async () => {
		...
		try {
			...
			toast.error(
				"Failed to start",
				error instanceof Error ? error.message : "Failed to start remediation",
			)
		} finally {
			setIsStarting(false)
		}
				...
				<section>
					<div className="bg-surface border border-border rounded-lg p-6 text-center">
						...
						<p className="text-sm text-muted-foreground max-w-md mx-auto">
							Remediation will be triggered automatically if the AI Review or
							Human Review finds issues that need to be fixed. You can also
							manually start remediation if needed.
						</p>
					</div>
				</section>
			)}
				...
				<section>
					<div className="bg-primary/5 border border-primary/20 rounded-lg p-6">
						...
						<p className="text-sm text-muted-foreground text-center">
							The agent is fixing the issues found during review. Once complete,
							it will return to AI Review for validation.
						</p>
					</div>
				</section>
			)}
			...
			{isCompleted && cycles.length === 0 && (
				<section>
					<div className="bg-success/5 border border-success/20 rounded-lg p-6 text-center">
						...
						<p className="text-sm text-muted-foreground max-w-md mx-auto">
							The review process completed without requiring any remediation
							cycles.
						</p>
					</div>
				</section>
			...
			{cycles.length > 0 && (
				<section>
					<h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3 flex items-center">
						<Wrench size={16} className="mr-2" /> Remediation Cycles (
						{cycles.length})
					</h3>
					<div className="space-y-3">
						{cycles.map((cycle) => (
							<RemediationCycleCard
								...
								isCurrentCycle={
									cycle.cycleNumber === currentCycleNumber && isInProgress
								}
							/>
						))}
					</div>
