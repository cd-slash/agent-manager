...
export function TaskTemplatesSettings() {
  ...
  const [editingTemplate, setEditingTemplate] = useState<TaskTemplateDoc | null>(null);
  const [deletingTemplate, setDeletingTemplate] = useState<TaskTemplateDoc | null>(null);
  ...
  const handleSeedTemplates = async () => {
    ...
    try {
      ...
      toast.error('Seed failed', error instanceof Error ? error.message : 'Failed to seed templates');
    } finally {
      setIsSeeding(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error('Save failed', error instanceof Error ? error.message : 'Failed to save template');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error('Delete failed', error instanceof Error ? error.message : 'Failed to delete template');
    }
  };
  ...
  const handleSetDefault = async (template: TaskTemplateDoc) => {
    try {
      ...
      toast.success('Default updated', `"${template.name}" is now the default template`);
    } catch (error) {
      toast.error('Update failed', error instanceof Error ? error.message : 'Failed to set default');
    }
  };
    ...
    <section>
      ...
      <div className="bg-surface border border-border rounded-lg p-4">
        <p className="text-sm text-muted-foreground mb-4">
          Task templates define the workflow phases for different types of tasks.
          Choose which phases to include and in what order tasks should progress.
        </p>

        {templates.length === 0 ? (
          <div className="text-center py-8">
            <FileStack size={48} className="mx-auto mb-4 text-muted-foreground opacity-50" />
            <p className="text-sm text-muted-foreground mb-4">
      ...
      {/* Create/Edit Modal */}
      <Dialog open={isCreateModalOpen || !!editingTemplate} onOpenChange={(open) => !open && handleCloseModal()}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {editingTemplate?.isBuiltin ? (
                ...
                <p className="text-sm text-muted-foreground mb-2">
                  Select which phases to include. Tasks will progress through phases in order.
                </p>
              )}
            ...
            <div className="bg-surface border border-border rounded-lg p-3">
              <Label className="text-xs text-muted-foreground uppercase">Preview</Label>
