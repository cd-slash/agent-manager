...
import { Bot, RefreshCw, Save, RotateCcw, ChevronDown, ChevronUp } from 'lucide-react';
...
const PERMISSION_MODES = [
  ...
  { value: 'plan', label: 'Plan Mode', description: 'Agent creates plan before executing' },
  { value: 'accept_edits', label: 'Accept Edits', description: 'Auto-accept file edits' },
  { value: 'full_auto', label: 'Full Auto', description: 'Auto-accept all actions' },
];
...
function PhaseConfigCard({ phase, config, builtinPrompt, onSave, isSaving }: PhaseConfigCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [model, setModel] = useState(config?.model || 'claude-sonnet-4-20250514');
  const [permissionMode, setPermissionMode] = useState(config?.permissionMode || 'default');
  const [promptTemplate, setPromptTemplate] = useState(config?.promptTemplate || builtinPrompt || '');
        ...
        <div className="border-t border-border p-4 space-y-4 bg-background/50">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Available variables: {'{{task.title}}'}, {'{{task.description}}'}, {'{{task.id}}'}, {'{{pr.number}}'}, {'{{pr.branch}}'}
            </p>
          </div>
...
export function PhaseDefaultsSettings() {
  ...
  const handleSavePhaseConfig = async (
    ...
    try {
      ...
      toast.success('Configuration saved', `${PHASE_DISPLAY_NAMES[phase]} agent settings updated`);
    } catch (error) {
      toast.error('Failed to save', error instanceof Error ? error.message : 'Failed to save configuration');
    } finally {
      setSavingPhase(null);
    }
  };
  ...
  return (
    <section>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Configure the default model and prompt for each agent phase. These settings can be overridden per-task.
      </p>
      <div className="space-y-3">
        {AGENT_PHASES.map((phase) => (
          <PhaseConfigCard
            ...
            builtinPrompt={builtinDefaults?.prompts?.[phase as keyof typeof builtinDefaults.prompts] || null}
