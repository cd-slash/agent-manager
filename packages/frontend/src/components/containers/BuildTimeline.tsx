...
const PHASE_INFO: Record<string, { name: string; description: string }> = {
  ...
  building_binary: { name: 'Building Binary', description: 'Compiling container-api' },
  building_image: { name: 'Building Image', description: 'Docker build in progress' },
  starting_container: { name: 'Starting Container', description: 'Container starting, Tailscale connecting' },
  deploying_binary: { name: 'Deploying Binary', description: "Copying binary to container" },
  starting_api: { name: 'Starting API', description: 'Starting container-api service' },
...
function PhaseIcon({ status, isActive }: { status: BuildPhase['status']; isActive: boolean }) {
  const iconSize = 18;
  ...
  switch (status) {
    ...
    case 'pending':
    default:
      return <Clock size={iconSize} className={cn('text-muted-foreground', isActive && 'text-blue-400')} />;
  }
}
    ...
    <div className="relative">
      ...
      <div className="space-y-1">
        {sortedPhases.map((phase, index) => {
          const info = PHASE_INFO[phase.phase] || { name: phase.phase, description: '' };
          const isActive = phase.phase === currentPhase && phase.status === 'in_progress';
