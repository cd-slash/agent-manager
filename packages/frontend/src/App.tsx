...
function App() {
	...
	const [selectedServerId, setSelectedServerId] =
		useState<Id<"servers"> | null>(null)
	const [selectedContainerId, setSelectedContainerId] =
		useState<Id<"containers"> | null>(null)
	...
	const renderContent = () => {
		...
		if (activeView === "servers") {
			if (selectedServerId && selectedServer) {
				return (
					<ServerDetailView server={selectedServer} containers={containers} />
				)
			}
			return (
				<ServerView
					servers={servers}
					onSelectServer={(id) => setSelectedServerId(id as Id<"servers">)}
				/>
			)
		}
			...
			if (selectedContainerId && selectedContainer) {
				return (
					<ContainerDetailView
						...
						onDeleted={() => setSelectedContainerId(null)}
					/>
				)
			}
			return (
				<ContainerView
					...
					onSelectContainer={(id) =>
						setSelectedContainerId(id as Id<"containers">)
					}
				/>
			)
		}
										...
										<h2 className="text-sm font-bold text-foreground flex items-center">
											...
											{selectedTask.prCreated && (
												<Badge variant="purple" className="ml-item text-[10px]">
													<GitPullRequest size={10} className="mr-compact" /> #
											...
											<Badge
												variant={
													selectedTask.category === "done"
														? "success"
														: selectedTask.category === "in-progress"
															? "info"
															: "secondary"
												}
