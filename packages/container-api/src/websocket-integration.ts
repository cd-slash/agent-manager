...
export class WebSocketIntegration {
	...
	private setupMessageHandlers(): void {
		// Execution commands
		this.connection.onMessage<ExecStartPayload>(
			"exec:start",
			async (payload, msg) => {
				console.log(
					`[ws] Received exec:start: ${payload.message.substring(0, 50)}...`,
				)
				await this.handleExecStart(payload, msg.correlationId)
			},
		)

		this.connection.onMessage<ExecAbortPayload>(
			"exec:abort",
			async (payload, msg) => {
		...
		this.connection.onMessage<AuthRequestPayload>(
			"auth:request",
			async (payload, msg) => {
		...
		this.connection.onMessage<AuthFlowStartPayload>(
			"auth:flow:start",
			async (_, msg) => {
		...
		this.connection.onMessage<AuthFlowCompletePayload>(
			"auth:flow:complete",
			async (payload, msg) => {
		...
		// Session commands
		this.connection.onMessage<SessionListPayload>(
			"session:list",
			async (_, msg) => {
		...
		this.connection.onMessage<SessionDeletePayload>(
			"session:delete",
			async (payload, msg) => {
	...
	private async handleExecAbort(
		payload: ExecAbortPayload,
		correlationId?: string,
	): Promise<void> {
		const success = this.processManager.abort(payload.processId)
		this.connection.send(
			"exec:aborted",
			{ processId: payload.processId, success },
			correlationId,
		)
	}
	...
	private async handleAuthFlowComplete(
		payload: AuthFlowCompletePayload,
		correlationId?: string,
	): Promise<void> {
		try {
