...
export class AuthManager extends EventEmitter {
	...
	async startWatching(): Promise<void> {
		console.log(
			"[auth] Auth manager initialized (env var based authentication)",
		)
	}
	...
	async startOAuthFlow(): Promise<OAuthStartResult> {
		...
		const url = await new Promise<string>((resolve, reject) => {
			...
			const timeout = setTimeout(() => {
				...
				reject(
					new Error(
						`Timeout waiting for OAuth URL. Output so far: ${outputState.allOutput.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, "")}`,
					),
				)
		...
		try {
			...
			const pollPtyOutput = async (): Promise<string | null> => {
				...
				for (let i = 0; i < 120; i++) {
					...
					if (outputToken) {
						console.log(
							`[auth] ✅ Token found in PTY output: ${outputToken.substring(0, 20)}...`,
						)
						return outputToken
					}
				}
			...
			const timeoutPromise = new Promise<{ exitCode: null; source: "timeout" }>(
				(resolve) => {
					setTimeout(() => {
			...
			if (result.type === "token" && result.token) {
				...
				if (outputToken) {
					console.log(
						`[auth] ✅ Token found in PTY output (final check): ${outputToken.substring(0, 20)}...`,
					)
				...
				if (!foundToken && outputState) {
					const cleanOutput = outputState.allOutput
						...
						.replace(/\x1b\]0;[^\x07]*\x07/g, "")
					console.log(
						`[auth] ⚠️ No token found. Full PTY output (${cleanOutput.length} chars):`,
					)
